...
{   
    ...
    Vector3f p1, p2, p3, v1, v2, v3;
    p1 = Vector3f(_v[0].x() - x, _v[0].y() - y, 0);
    p2 = Vector3f(_v[1].x() - x, _v[1].y() - y, 0);
    p3 = Vector3f(_v[2].x() - x, _v[2].y() - y, 0);
    v1 = (_v[1] - _v[0]).head(3);
    v2 = (_v[2] - _v[1]).head(3);
    v3 = (_v[0] - _v[2]).head(3);
    bool res = sign(p1.cross(v1)[2]) == sign(p2.cross(v2)[2]) && sign(p1.cross(v1)[2]) == sign(p3.cross(v3)[2]);
    return res;
}
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    for (int i = x_min; i <= x_max; i++) {
        for (int j = y_min; j <= y_max; j++) {
            if (insideTriangle(i+0.5f, j+0.5f, v.data())) {
